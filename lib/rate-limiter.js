 ...
 * This implementation uses an in-memory Map which will NOT work correctly
 * in serverless environments like Vercel, AWS Lambda, or similar platforms
 * where each request may be handled by a different instance.
 *
 * For production use on serverless platforms, you MUST replace this with:
 * - Redis (via @upstash/redis for serverless compatibility)
 * - Vercel KV
 * - DynamoDB
 * - Or another distributed rate limiting solution
 *
 * This code is suitable ONLY for:
 * - Traditional server deployments (single instance or sticky sessions)
 * - Development/testing
 * - Learning purposes
 */
 ...
 * Replace with Redis or distributed cache for production
 */
